...
module.exports = function (io) {
  io.on('connection', function (socket) {
    ...
    socket.on('connected', function (data) {
      connection.query(`
        ...
        if (err) {
          ...
          socket.to(onlineUsers[data.id]).emit('connectedFriends', {
            sucess: false,
            friends: null
          });
        } else {
          if (rows) {
            ...
            var n = rows.length; //console.log(findUser(connectedFriends));
      ...
      connection.query(`
        ...
        rows.forEach(e => {
          connection.query(` INSERT INTO messages (to_user, from_user, message, timestamp) VALUES (${e.id}, 1, '${data.message}', ${data.timestamp})`, function (err3, result) {
            ...
            connection.query(`    
                            INSERT INTO chats (user_1, user_2, last_message)
                            SELECT * FROM (SELECT 1, ${e.id}, ${result.insertId} ) AS tmp
                            WHERE NOT EXISTS (
    ...
    socket.on('disconnect', function (socket) {
      ...
      delete resOnlineUsers[socket.id]; //console.log(onlineUsers[socket.id]);
    });
