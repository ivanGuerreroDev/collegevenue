...
router.post("/sendFriendRequest", function(req, res){
    connection.query(`
        ...
        WHERE user = ${req.body.friend} AND blocked = ${req.body.user} 
    `,function(err3,rows3){
        if(!rows3[0]){
            connection.query(`
            ...
            return res.json({valid:false, error:'The user has blocked you°'}) 
        }
    })
...
router.post("/addFriend", function(req, res){
    connection.query(`
        ...
        SELECT * FROM (SELECT ${req.body.user}, ${req.body.friend}) AS tmp
        WHERE NOT EXISTS (
            SELECT user_id FROM friends WHERE user_id = ${req.body.user}
        ) 
        LIMIT 1
    `,function(err,rows){
        if(err){
    ...
    connection.query(`
        INSERT INTO friends (user_id, friend)
        SELECT * FROM (SELECT ${req.body.friend}, ${req.body.user}) AS tmp
        WHERE NOT EXISTS (
            SELECT user_id FROM friends WHERE user_id = ${req.body.friend}
        ) LIMIT 1
    `,function(err,rows){
        if(err){
            console.log(err)
    ...
    connection.query(`
        INSERT INTO follows (user_id, follow)
        SELECT * FROM (SELECT ${req.body.user}, ${req.body.friend}) AS tmp
        WHERE NOT EXISTS (
            SELECT user_id FROM follows WHERE user_id = ${req.body.user}
        ) LIMIT 1                
    `,function(err,rows){
        if(err){ 
            console.log(err)
    ...
    connection.query(`
        ...
        SELECT * FROM (SELECT ${req.body.friend}, ${req.body.user}) AS tmp
        WHERE NOT EXISTS (
            SELECT user_id FROM follows WHERE user_id = ${req.body.friend}
        ) LIMIT 1                
    `,function(err,rows){
        if(err){ 
            console.log(err)
    ...
    return res.json({valid:true, result:'Friend added'})
})
