...
router.post("/loginAdmin", function(req, res, next) {
  passport.authenticate('local', function(err, user, info) {
    if (err) { return res.json({valid:false,  message:'err'}) }
    if (!user) { return res.json({valid:false,  message:'User no encontrado'}) }
    req.logIn(user, function(err) {
      if (err) { return res.json({valid:false,  message:'err'}); }
      return res.json({valid:true,  message:'logged', user:user});
    });
  })(req, res, next)
});
...
router.post("/register", function(req, res, next) {
  ...
  connection.query(`SELECT * FROM domains WHERE domain = '${domain}'`,function(error,dominios){
    if(error) {console.log(error); return res.json({valid:false, error: 'Domain of email cant be register'})}
      ...
      return res.json({valid:false, error: 'Domain of email cant be register'})
    }
  })
...
router.post('/findUser/', /*isLoggedIn,*/ function(req, res, next) {
  ...
  AND users.id NOT IN 
  ( SELECT user FROM blockeds WHERE user=users.id AND blocked=${req.body.user} )
  `,function(err,rows){
...
function sendCode(email, code){
  ...
  let mailOptions = {
    ...
    subject: "Verification Code from College Venue", // Subject line
